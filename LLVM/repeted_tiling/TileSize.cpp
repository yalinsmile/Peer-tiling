...
namespace {
	struct TileSize : public ModulePass {
		static char ID;
		TileSize() : ModulePass(ID) {}
		...
		virtual bool runOnModule(Module &M) {
			...
			Function* F1 = M.getFunction("sgemm");
...
/*****************************************the part for have loop 't'******************************************************/
                 	         if(F1 && reorgA && reorgB) {       
                                ...
                                // Value * bb=builder3.CreateCall(reorgA);
...
char TileSize::ID = 0;
static RegisterPass<TileSize> X("tilesize","get_tilesize pass");
